...
export const CommitsGrid = ({ text }: { text: string }) => {
  const [cellsData, setCellsData] = React.useState<CellData[]>([]);
  ...
  React.useEffect(() => {
    setIsClient(true);
    // Generate random data for all cells after hydration
    const totalCells = gridWidth * gridHeight;
    const newCellsData: CellData[] = Array.from({ length: totalCells }, () => ({
      delay: getRandomDelay(),
      color: getRandomColor(),
      shouldFlash: getRandomFlash(),
    }));
  ...
  return (
    ...
    >
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        const shouldFlash = isClient && cellData && !isHighlighted && cellData.shouldFlash;
        ...
        return (
          <div
            ...
            style={
              {
                animationDelay: isClient && cellData ? cellData.delay : "0s",
                "--highlight": isClient && cellData ? cellData.color : "#48d55d",
              } as CSSProperties
            }
          />
        );
